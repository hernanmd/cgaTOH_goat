# Author: Daniel Gozckzynsky, Hernán Morales Durand
# Date: 
# Input: 
#     cgaTOH output files .homozygousruns
#     Directory with .PEDMAP files
# Output: 
#     FROH Plots
...
##########################################################
# If you already know 
#  1) Your genome is annotated in UCSC
#  2) The name of the annotation library
##########################################################

# Uncomment for your species:
# library(BSgenome.Btaurus.UCSC.bosTau8) 
# library(BSgenome.????.UCSC.equCab2) 
# library(BSgenome.Sscrofa.UCSC.susScr11)
# library(BSgenome.????.UCSC.oviAri3)


###########################################################
# If you don't know if your genome is NOT annotated in UCSC
###########################################################

##############################################
# See all the available genomes in UCSC
##############################################
library("BSgenome")
?ucscGenomes
ucscGenomes(organism = TRUE)
available.genomes(splitNameParts = FALSE, type = getOption("pkgType"))

# Example: Get the table names available for bosTau8 genome
supportedUCSCtables("bosTau8")


##################################################################
# If you already know your species genome is NOT annotated in UCSC
##################################################################

# Specify a file with chromosome lengths manually
chrLengthsFilename <- "chr_lengths.txt"
...
# Import cgaTOH output files
ff <- list.files(
           full.names = TRUE, 
           pattern = ".homozygousruns")
View(ff)
# Remove 0.5 Mb files if present
# ff = ff[!ff %in% grep('500000', ff, value = T)]

# Extract file name from ff table with each row being a file name ex: 29_3_8000000_0_0.homozygousruns
myfilelist <- lapply(ff, read.table, stringsAsFactors = F, sep = ",", header = T)
names(myfilelist) <- unlist(lapply(ff, 
           function(x) 
              str_match(x,cgaTOHOutfileRegex)))

# Build a vector with each chromosome number repeated by the number of files per chromosome in the cgaTOH output files
vector <- rep(1:maxChr, each = 5)
# Group file list vector by repetitions in vector chromosome
# (generar lista de listas por cromosoma y muestra)
...
# If you already know your species genome is NOT annotated in UCSC
##################################################################
# Load text file with chromosome lengths
chrLengths <- read.table(
  chrLengthsFilename, 
  stringsAsFactors = F, 
  header = F)
cLengths <- chrLengths$V3
...
###########################################################
#
# Plot FROH by chromosome
#
###########################################################

ggplot(
  fROHSxChr, 
  aes(x = Chromosome, y = FROH, fill = Group)) + 
    theme_classic() + 
    geom_boxplot() + 
...
ggplot(
    ...
    aes(x=L1, y=value)) + 
      ...
      scale_x_discrete(name ="Sample (Group LI)", 
      limits = mixedsort(unique(melt$L1))) +
      scale_y_continuous(
        name = "ROH Length", 
...
ggplot(data=mapL) + 
  geom_bar(aes(x=Pos, y=x,colour = factor(mapL$Chr)), stat="identity") + 
  facet_grid(. ~ Chr, scales = "free") +
  labs(x="SNP position by chromosome",y="Number of ROH") +
